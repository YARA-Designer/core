...
from sqlalchemy import Column, Integer, JSON, DateTime, VARCHAR, Boolean, Table, ForeignKey
...
# Helper table that is used for many-to-many relationship.
...
class YaraRuleDB(Base):
    ...
    tags = relationship('YaraTagDB', secondary=tags, lazy='subquery', backref=backref('pages', lazy=True))
    meta = Column(JSON)
    strings = Column(JSON)
    ...
    def __init__(self, name: str, thehive_case_id: str = None, tags: list = None, meta: list = None,
                 strings: list = None, condition: str = None, namespace: str = None,
                 compiled_path: str = None, source_path: str = None):
    ...
    def as_dict(self):
        return {
            "name": self.name,
            "case_id": self.thehive_case_id,
            "tags": [t.name for t in self.tags], 
            "added_on": self.added_on,
            "last_modified": self.last_modified,
            "yara_file": self.source_path,
